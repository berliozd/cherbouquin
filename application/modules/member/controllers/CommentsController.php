...
class Member_CommentsController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $ajaxContext->addActionContext('add-userbook-comment', 'html')
                ->initContext();
    }
    ...
    public function addUserbookCommentAction() {
        ...
        if ($globalContext->getConnectedUser()) {
            ...
            if (UserbookCommentDao::getInstance()->add($comment)) {
                ...
                if (($reviewUser->getId() != $globalContext->getConnectedUser()->getId()) &&
                        ($reviewUser->getSetting()->getEmailMe() == \Sb\Helpers\UserSettingHelper::EMAIL_ME_YES )) {
                ...
                $this->forward("get-reviews-page", "book", "default", array("key" => $bookId,
                    "param" => $reviewPageId,
                    "format" => "html"));
            }
        } else
